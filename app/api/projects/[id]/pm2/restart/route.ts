/**
 * POST /api/projects/[id]/pm2/restart
 * Restart a PM2 process for a specific project
 */

import { NextRequest, NextResponse } from 'next/server';
import { withAdminAuth, logAuditEvent } from '@/lib/auth-middleware';
import { getProjectById } from '@/config/projects';
import { restartPM2Process } from '@/lib/system-commands';
...
export async function POST(
  ...
  return withAdminAuth(request, async (userInfo) => {
    ...
    try {
      ...
      // Log the action
      logAuditEvent(userInfo.email, 'PM2_RESTART', `Project: ${id}, Process: ${processName}`);
