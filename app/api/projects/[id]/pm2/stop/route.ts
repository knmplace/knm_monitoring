/**
 * POST /api/projects/[id]/pm2/stop
 * Stop a PM2 process for a specific project
 */

import { NextRequest, NextResponse } from 'next/server';
import { withAdminAuth, logAuditEvent } from '@/lib/auth-middleware';
import { getProjectById } from '@/config/projects';
import { stopPM2Process } from '@/lib/system-commands';
...
export async function POST(
  ...
  return withAdminAuth(request, async (userInfo) => {
    ...
    try {
      ...
      const result = await stopPM2Process(processName);

      // Log the action
      logAuditEvent(userInfo.email, 'PM2_STOP', `Project: ${id}, Process: ${processName}`);
